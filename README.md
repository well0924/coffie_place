...
- 첫 개인 프로젝트를 하면서 **주제 선정부터 프로젝트 배포까지의 전 과정**을 거치면서 **전반적인 개발의 흐름**을 **알 수 있는 기회**였던 
  프로젝트였습니다.


- 스프링의 **MVC패턴과 데이터베이스의 기본적인 쿼리를 이해**하고 써 볼 수 있었던 좋은 기회였고, 개발을 진행을 하면서 **중복되는 코드 및 
  기능들을 따로 만들어서 생산성**을 높일 수 있었습니다.


- 사용자들의 편의를 위해서 게시판에 **에디터를 작용해서 편의성을 높였고,** **관리자 입장에서 가게 목록을 보기 편하게 하기 위해서 apache poi를 
  사용해서 가게 데이터를 엑셀 파일**로 만들어서 개발을 하는 것도 중요하지만 **사용자의 입장에서 편의성을 높이기 위한 방법에 대해서 생각**할 수 있는 기회였습니다.


- 개발을 진행하면서 **작성한 api를 문서화를 하기** 위해서 **스웨거를 작성**을 하기로 했는데 스프링 시큐리티를 접목을 하면서 보안 설정에 대해 
  알지를 못하고 겉핥기식으로 알아서 **다음에 할 때에는 정확하게 라이브러리에 대한 내용을 숙지**를 해야겠고, **api 문서를 작성을 하면서 개발자간의 커뮤니케이션**에 대해서 알 수 있었다.


- 프로젝트가 완료가 되었다고 해서 끝나는 것이 아닌 **jpa로 배운 뒤 jpa로 전환**을 해 볼 예정입니다.
